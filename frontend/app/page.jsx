...
function AssetsView({ bundle, token, refresh, user }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setMessage(`Workflow complete: ${payload.data.workflow.youtubeResults} YouTube results, ${payload.data.workflow.detections} detections, ${payload.data.workflow.evidencePackets} evidence packets.`);
      await refresh();
    } catch (err) {
...
function SettingsView({ token, user }) {
  ...
  useEffect(() => {
    async function load() {
      const [profileRes, orgRes, notificationRes] = await Promise.all([
